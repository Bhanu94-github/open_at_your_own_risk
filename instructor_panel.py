...
def instructor_dashboard():
    ...
    with tab1:
        ...
        if st.button("🔁 Bulk Reset All Tokens to 15"):
            for s in filtered_students:
                ...
                log_token_history(username, instructor_username, "Bulk Reset to 15", 15)
            st.success("All filtered students reset to 15 tokens.")
            st.rerun()
        ...
        for student in filtered_students:
            ...
            with st.expander(f"👤 {name} ({username})"):
                st.write(f"🎯 **General Tokens**: `{tokens}`")
                ...
                if col3.button("🔁 Reset to 15", key=f"reset_{username}"):
                    access_col.update_one(
                        {"username": username},
                        {
                            "$set": {
                ...
                st.markdown("#### 🎓 Module-wise Token Management")
                for module in ["Text_to_Text", "Voice_to_Voice", "Face_to_Face"]:
                    st.write(f"**{module.replace('_', ' ').title()} Tokens**: `{ai_tokens.get(module, 0)}`")
                    mcol1, mcol2, mcol3 = st.columns(3)
                    if mcol1.button("➕", key=f"{module}_inc_{username}"):
                        access_col.update_one(
                            {"username": username},
                            {"$inc": {f"ai_tokens.{module}": 1}}
                        )
                        log_token_history(username, instructor_username, "Module Token Increment", 1, module)
                        st.rerun()
                    if mcol2.button("➖", key=f"{module}_dec_{username}"):
                        if ai_tokens.get(module, 0) > 0:
                            access_col.update_one(
                                {"username": username},
                                {"$inc": {f"ai_tokens.{module}": -1}}
                            )
                            log_token_history(username, instructor_username, "Module Token Decrement", -1, module)
                            st.rerun()
                    if mcol3.button("🔁 Reset to 15", key=f"{module}_reset_{username}"):
                        access_col.update_one(
                            {"username": username},
                            {"$set": {f"ai_tokens.{module}": 15}}
                        )
                        log_token_history(username, instructor_username, "Module Reset to 15", 15, module)
                        st.rerun()

    # --------------------- Token Logs ---------------------
    with tab2:
        st.subheader("📄 Token Action Logs")
        logs = list(db["token_logs"].find({"instructor": instructor_username}).sort("timestamp", -1).limit(100))
        if not logs:
            ...
            for log in logs:
                st.write(
                    ...
                    f"👤 **{log['student']}** | "
                    f"📌 Module: `{log.get('module', 'general')}` | "
                    f"✍️ {log['action']} | 🔢 Changed: `{log['tokens_changed']}`"
                )
    ...
    with tab3:
        ...
        fig1 = px.bar(token_df, x="username", y=["tokens_left", "Text-to-Text", "Voice-to-Voice", "Face-to-Face"],
